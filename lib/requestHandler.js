...
function getWsHandler(userRule, recorder, wsClient, wsReq) {
  ...
  try {
    ...
    const proxyWs = new WebSocket(wsUrl, serverInfo.protocol, {
    ...
    if (recorder) {
      Object.assign(resourceInfo, wsReqInfo);
      resourceInfoId = recorder.appendRecord(resourceInfo);
    }
    ...
    */
    ...
    proxyWs.onmessage = (event) => {
      ...
      userRule.beforeSendWsMessageToClient(wsReqInfo, event).then(
        () => {},
        (err) => {}
      );
    }
    ...
    wsClient.onmessage = (event) => {
      ...
      userRule.beforeSendWsMessageToServer(wsReqInfo, event).then(
        () => {},
        (err) => {}
      );
    }
